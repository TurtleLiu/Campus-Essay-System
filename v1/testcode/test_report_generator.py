...
def calculate_code_ratio():
    """计算测试代码与应用功能代码的比例"""
    # 应用功能代码
    app_file = os.path.join(os.path.dirname(__file__), '..', 'elementary_essay_tutor_app.py')
...
| **总计** | **4个文件** | **{test_results['total']}** | **{code_ratio['test_lines']}** |"""
...
| **总计** | **-** | **{test_results['total']}** | **{test_results['passed']}** | **{test_results['failed']}** | **{test_results['errors']}** | **{test_results['success_rate']:.1f}%** |"""
    ...
    for file_name in ['test_basic_functions.py', 'test_llm_functions.py', 'test_system_integration.py', 'test_acceptance.py']:
